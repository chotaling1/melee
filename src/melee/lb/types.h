...
#ifdef MELEE_PORT
    /* Color script words are host-order u32s in the port (see CmdUnion);
     * these are the same fields with LSB-first layouts. */
    struct {
        u8 a, b, g, r;
    } light_color;
    struct {
...
#ifdef MELEE_PORT
/* Script words are converted to host byte order when their DAT is loaded,
 * so the port reads them through LSB-first equivalents of the declarations
 * below, generated by port/tools/gen_cmd_layouts.py. */
#include <port_gen/lb_cmd.h>
#else
struct Command_00 {
    u32 code : 6;
    u32 value : 26;
...
/// Index of byte @p i / half-word @p i of a big-endian script word, for code
/// that reads script words through u8 or u16 pointers. The port stores the
/// words in host (little-endian) order.
#ifdef MELEE_PORT
#define CMD_BYTE(i) (3 - (i))
#define CMD_HALF(i) (1 - (i))
#else
#define CMD_BYTE(i) (i)
#define CMD_HALF(i) (i)
#endif
